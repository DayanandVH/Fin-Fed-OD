...
def scaler_mixdatatype(trainX, trainy, test_sets_data, test_sets_label, num_features, cat_features):
    ...
    trainX, valX, trainy, valy = train_test_split(data_with_onehot, trainy, test_size=0.2, random_state=seed, stratify=trainy)
...
def read_and_preprocess_data(train, test_sets_data_label, ds_name):
    ...
    if ds_name == 'ann_thyroid':
        num_features = ['C1', 'C17', 'C18', 'C19', 'C20', 'C21']
        cat_features = ['C2', 'C3', 'C4', 'C5', 'C6', 'C7', 'C8', 'C9', 'C10', 'C11', 'C12', 'C13', 'C14', 'C15', 'C16']
    ...
    return trainX, test_sets_data, valX, trainy, test_sets_label, valy, std_scaler_ds, cat_uniqval_count, label_col_name
...
def load_datasets(batch_size, ds_name, folder_name):
    ...
    # train_sets_data_label = [train_type1, train_type2]
    # test_sets_data_label = [test_type1, test_type2]
    
    # ds1_name = 'credit_default'

    dim1 = []
    for train_set, test_set in zip(train_sets_data_label, test_sets_data_label):
        trainX, test_sets_data, valX, trainy, test_sets_label, valy, std_scaler_ds, cat_uniqval_count, label_feature_ds1 = read_and_preprocess_data(train_set, [test_set], ds_name)
        dim1.append(trainX.shape[1])
    max_dim1 = np.max(dim1)
    input_dim = max_dim1
    ...
    for train_set in train_sets_data_label:
        trainX, test_sets_data, valX, trainy, test_sets_label, valy, std_scaler_ds, cat_uniqval_count, label_feature_ds1 = read_and_preprocess_data(train_set, test_sets_data_label, ds_name)
        cat_uniqval_loader.append(cat_uniqval_count)
        test_sets = []
        test_sets_data1 = []
        if trainX.shape[1] < max_dim1:
            trainX = np.concatenate((trainX, np.zeros((len(trainX), int(max_dim1-trainX.shape[1])))), axis=1)
            valX = np.concatenate((valX, np.zeros((len(valX), int(max_dim1 - valX.shape[1])))), axis=1)
            for ind, i in enumerate(test_sets_data):
                i = np.concatenate((i, np.zeros((len(i), int(max_dim1 - i.shape[1])))), axis=1)
                i = pd.DataFrame(i)
    ...
    return trainloaders, valloaders, testloaders, full_train_data, trainy_sets, valy_sets, testy_sets, cat_uniqval_loader, input_dim
...
def od_model_for_latent_classification(trainX, test_sets, trainy, test_sets_label, cid, epoch, op_path):
    ...
    result_outliers.loc[len(result_outliers)] = ['Avg_Score_of_Other_Outliers', np.mean(result_other_outliers['Precision'].values), np.mean(result_other_outliers['Recall'].values),
                                                                         np.mean(result_other_outliers['f1-score'].values), np.mean(result_other_outliers['Pr-auc'].values), np.mean(result_other_outliers['Roc-auc'].values), np.mean(result_other_outliers['Average_Precision_Score'].values)]
    ind1 = int(int(cid)+1)
...
def train(cid, server_round, model, trainloader, valloader, cat_uniqval_count, epochs: int, batch_size:int, verbose=False):
    ...
    for epoch in range(1, epochs + 1):
        ...
        val_rec_error_bce = torch.zeros(valloader.shape[0]).to(device)
        curr_col_ind = 0
        # for cat_attr in cat_features:
        for ind, cat_ind_loc in enumerate(cat_uniqval_count):
            # compute reconstruction error of categorical and numerical parts
            val_rec_error_bce_single_attr = reconstruction_BCE(input=rec_val[:, curr_col_ind:cat_ind_loc + curr_col_ind], target=valloader[:, curr_col_ind:cat_ind_loc + curr_col_ind].argmax(axis=1))
            val_rec_error_bce += val_rec_error_bce_single_attr
            curr_col_ind += cat_ind_loc
        # for num_attr in numerical_attributes_scaled:
        val_rec_error_mse = reconstruction_MSE(input=rec_val[:, int(sum(cat_uniqval_count)):], target=valloader[:, int(sum(cat_uniqval_count)):]).sum(dim=1)
        # compute total reconstruction error
...
def test(model, server_round, trainX, testloader, trainy, testy, cid, cat_uniqval_count, epoch):
    ...
    # rec_err_train = np.abs(rec_err_train.detach().numpy())
    train_rec_error_bce = torch.zeros(trainX.shape[0]).to(device)
    curr_col_ind = 0
    # for cat_attr in cat_features:
    for ind, cat_ind_loc in enumerate(cat_uniqval_count):
        # compute reconstruction error of categorical and numerical parts
        train_rec_error_bce_single_attr = reconstruction_BCE(input=rec_train[:, curr_col_ind:cat_ind_loc + curr_col_ind], target=trainX[:, curr_col_ind:cat_ind_loc + curr_col_ind].argmax(axis=1))
        train_rec_error_bce += train_rec_error_bce_single_attr
        curr_col_ind += cat_ind_loc
    # for num_attr in numerical_attributes_scaled:
    train_rec_error_mse = reconstruction_MSE(input=rec_train[:, int(sum(cat_uniqval_count)):], target=trainX[:, int(sum(cat_uniqval_count)):]).sum(dim=1)
    # compute total reconstruction error
    rec_err_train = train_rec_error_bce + train_rec_error_mse
    # rec_err_train = rec_err_train.detach().numpy()
    rec_err_train = rec_err_train.detach().cpu().numpy()
    ...
    for ind, i in enumerate(testloader):
        ...
        test_rec_error_bce = torch.zeros(testX.shape[0]).to(device)
        curr_col_ind = 0
        # for cat_attr in cat_features:
        for ind, cat_ind_loc in enumerate(cat_uniqval_count):
            # compute reconstruction error of categorical and numerical parts
            test_rec_error_bce_single_attr = reconstruction_BCE(input=rec_test[:, curr_col_ind:cat_ind_loc + curr_col_ind], target=testX[:, curr_col_ind:cat_ind_loc + curr_col_ind].argmax(axis=1))
            test_rec_error_bce += test_rec_error_bce_single_attr
            curr_col_ind += cat_ind_loc
        # for num_attr in numerical_attributes_scaled:
        test_rec_error_mse = reconstruction_MSE(input=rec_test[:, int(sum(cat_uniqval_count)):], target=testX[:, int(sum(cat_uniqval_count)):]).sum(dim=1)
        # compute total reconstruction error
...
class FlowerClient(fl.client.NumPyClient):
    def __init__(self, cid, model, trainloader, valloader, testloader, train_data, trainy, valy, testy, cat_uniqval_count):
        self.cid = cid
        self.model = model
        ...
        self.valy = valy
        self.testy = testy
        self.cat_uniqval_count = cat_uniqval_count
    ...
    def fit(self, parameters, config):
        ...
        train(self.cid, server_round, self.model, self.trainloader, self.valloader, self.cat_uniqval_count, epochs=epochs, batch_size=batch_size)
    ...
    def evaluate(self, parameters, config):
        ...
        set_parameters(self.model, parameters)
        test_loss, train_rec_err_per_sample, test_rec_err_per_sample = test(self.model, server_round, self.train_data, self.testloader, self.trainy, self.testy, self.cid, self.cat_uniqval_count, epochs)
...
def client_fn(cid) -> FlowerClient:
    ...
    # Load data
    trainloaders, valloaders, testloaders, full_train_data, trainy_sets, valy_sets, testy_sets, cat_uniqval_loader, input_dim = load_datasets(batch_size, ds_name, input_data_path)
    ...
    cat_uniqval_count = cat_uniqval_loader[int(cid)]
    
    # Load model
    model = AutoEncoder([input_dim]+model_architecture[0], model_architecture[1]+[input_dim]).to(device)

    # Create a  single Flower client representing a single organization
    return FlowerClient(cid, model, trainloader, valloader, testloader, train_data, trainy, valy, testy, cat_uniqval_count)
...
model_architecture = [[128,64],[64,128]]
model_foldername = 'model_128_64'
round_list = [10, 50, 100, 200]
fl_rounds = 200
...
for i in range(1,2):
    ...
    if ds_index == 1:
        ...
        input_data_path = '../data/run' + str(run_no) + '_synthetic_outlier_CD_dataset_outliers_3per_corr_cat_cols_25_corr_num_cols_25_per_types_1missing_2lessprob_3gaussian_4scaling'
    ...
    if ds_index == 2:
        ...
        input_data_path = '../data/run' + str(run_no) + '_synthetic_outlier_AD_dataset_outliers_3per_corr_cat_cols_25_corr_num_cols_25_per_types_1missing_2lessprob_3gaussian_4scaling'
